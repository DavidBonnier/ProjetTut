	...
	for ( int i = 0 ; i<=1 ; ++i)
	{
		...
		for (int j=0 ; j<tableauFigure.size(); ++j)//A changer si list
		{
			...
			if(DroiteCourante!=NULL)
			{
				...
				angletemp = 180* qAcos ((VecteurO.x()*VecteurCourant.x() + VecteurO.y()*VecteurCourant.y())/(sqrt(VecteurO.x()*VecteurO.x() + VecteurO.y()*VecteurO.y()) * sqrt(VecteurCourant.x()*VecteurCourant.x() + VecteurCourant.y()*VecteurCourant.y())))/PI;
