...
void User::RemoveUtil()
{
	...
	if(ok && !id.isEmpty() )
	{
		...
		if (id_correct) //si l'identifiant saisi existe
		{
			...
			//on resiez le fichier (une ligne de moins) et on écrit les identifiants gardés
			file.resize(0);
			flux << temp;
			file.close();
			
			QMessageBox* suppression_ok = new QMessageBox(QMessageBox::NoIcon, tr("Gestion des utilisateurs "), "La suppresion s'est déroulée avec succès. ");
			suppression_ok->setIconPixmap(QPixmap("Resources/suppr_uti.png"));
			suppression_ok->show();
		}
		else
		{
